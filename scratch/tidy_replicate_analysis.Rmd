...
summary_df %>% 
    ...
    ggplot() +
    geom_bar(aes(x = metric, y = value, fill = lab), 
             position = "dodge", stat = "identity") +
...
summary_df <- changeseq_df %>%
    ...
    filter(summary_df, Substitution_Only == TRUE) %>%
        arrange(desc(Nuclease_Read_Count)) %>%
        group_by(changeseq_run, lab, target_site) %>%
        summarise(total_on_target_count = sum(on_target),
                  on_target_top_off_target_ratio = total_on_target_count/) ## how do I reference the first row Nuclease Read Count value here?
...
summary_df <- changeseq_df %>%
    ...
    filter(summary_df, Substitution_Only == TRUE) %>%
      group_by(changeseq_run, lab, target_site, Site_Substitution_Number) ## something I'm doing here isn't correct
