...
off_target_seq_align_df %>% 
  ...
  labs(x = "Mismatch Type", y = "Read Count", fill = "Lab")
```
...
substitution_readcount_df %>% 
  ggplot() + 
  geom_point(aes(x = Site_Substitution_Number, 
                 y = substitution_read_count, 
                 fill = lab), 
             shape = 21, alpha = 0.25) + 
  facet_wrap(~target_site, scales = "free_y") + 
  theme_bw()
...
```{r}
ggplot(substitution_readcount_df) + 
    geom_bar(aes(x = Site_Substitution_Number, 
                 y = substitution_read_count, fill = lab), 
             position = "dodge", stat = "identity") +
...
(nsn_v_nnn_all <- off_target_scatter_df %>% 
  pivot_wider(names_from = "lab", values_from = "Nuclease_Read_Count", values_fill = 0) %>% 
  ggplot() + geom_point(aes(x = NSN, y = NNN), 
                        color = "darkblue", fill = "darkorange",
                        shape = 21, alpha = 0.25) + 
  scale_x_log10() + 
  scale_y_log10() + 
...
(nss_v_nnn_all <- off_target_scatter_df %>% 
  pivot_wider(names_from = "lab", values_from = "Nuclease_Read_Count", values_fill = 0) %>% 
  ggplot() + geom_point(aes(x = NSS, y = NNN), 
                        color = "darkblue", fill = "darkorange",
                        shape = 21, alpha = 0.25) + 
  scale_x_log10() + 
  scale_y_log10() + 
