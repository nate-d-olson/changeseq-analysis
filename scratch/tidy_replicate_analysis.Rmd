...
where 1000 is the number you want to sample.
...
off_target_seq_align_df <- summary_df%>%    
    group_by(changeseq_run, lab, target_site) %>%
    summarise(total_substitution_only_seqs = sum(Sub_Only),
...
* Paneled bar plot showing off-target sequence alignment details: number of sequences that fit into 3 categories
```{r}
off_target_seq_align_df %>% 
...
* Scatter plot of read counts per substitution number for each sample
```{r}
substitution_readcount_df %>% 
  ggplot() + 
  ...
  theme_bw()
```
*Bar plot of read counts per subsitution number for each sample
```{r}
ggplot(substitution_count_df) + 
    geom_bar(aes(x = Site_Substitution_Number, y = substitution_read_count, fill = lab), 
             position = "dodge", stat = "identity") +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 90)) + 
    labs(x = "Substitution Number", y = "Read Count", 
         fill = "Lab")
```
...
## Off-Target Genomic Coordinate Comparison Between Labs
...
Figures 
1. venn diagram comparing replicates - genomic coordinates/ seq (check for multiple seqs)
2. Figure 1i - read count scatter plot rep1 vs rep2 log10 scales - only overlapping, and all
3. excel point plots


```{r}
glimpse(off_target_df)
```
...
venn <- off_target_df %>%
  ...
  mutate(NSN_NSS = if_else(is.na(NSN) | is.na(NSS), 0, 1),
         NSN_NNN = if_else(is.na(NSN) | is.na(NNN), 0, 1),
         NSS_NNN = if_else(is.na(NSS) | is.na(NNN), 0, 1)) %>%
  group_by(lab, target_site) %>%
  sumarise(NSN_NSS_overlap = sum(NSN_NSS),
           NSN_NNN_overlap = sum(NSN_NNN),
           NSS_NNN_overlap = sum(NSS_NNN))
...
## will finish when I have the above code worked out
ggplot(data = venn)
...
#### All Off-Target Coordinates
__NOTE__ R squared values cannot be plotted/calculated because of the induction of zero values where coordinates are not shared between replicates

NSN vs. NSS: all off-targets
```{r}
(nsn_v_nss_all <- off_target_scatter_df %>% 
   ## values_fill make empty entries value of 0
  pivot_wider(names_from = "lab", values_from = "Nuclease_Read_Count", values_fill = 0) %>% 
  ggplot() + geom_point(aes(x = NSN, y = NSS), 
                        color = "darkblue", fill = "darkorange",
                        shape = 21, alpha = 0.25) + 
  scale_x_log10() + 
  scale_y_log10() + 
...
NSN vs. NNN: all off-targets
```{r}
(nsn_v_nnn_all <- off_target_scatter_df %>% 
  pivot_wider(names_from = "lab", values_from = "Nuclease_Read_Count", values_fill = 0) %>% 
  ggplot() + geom_point(aes(x = NSN, y = NSS), 
                        color = "darkblue", fill = "darkorange",
                        shape = 21, alpha = 0.25) + 
...
NSS vs. NNN: all off-targets
```{r}
(nss_v_nnn_all <- off_target_scatter_df %>% 
  pivot_wider(names_from = "lab", values_from = "Nuclease_Read_Count", values_fill = 0) %>% 
  ggplot() + geom_point(aes(x = NSN, y = NSS), 
                        color = "darkblue", fill = "darkorange",
                        shape = 21, alpha = 0.25) + 
...
Paneled view of pairwise lab comparisons of all off-target genomic coordinates and their respective read counts.
```{r fig.height= 6}
ggpubr::ggarrange(nsn_v_nss_all, nsn_v_nnn_all, nss_v_nnn_all, ncol = 3)
```
