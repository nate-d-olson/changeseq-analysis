...
file_list <- as.list(list.files(path = here("data-raw"), 
                           ...
                           recursive = TRUE) %>% 
    str_remove(., here("data-raw")))

## make that into a data frame
changeseq_metadata_df <- do.call(rbind.data.frame, file_list)
## add col names
colnames(changeseq_metadata_df) <- "file_path" 

changeseq_metadata_df <- changeseq_metadata_df %>% 
  mutate( lab = substr(file_path, 2, 5), 
          changeseq_run = (str_split(file_path, "/")[[1]][3]))
